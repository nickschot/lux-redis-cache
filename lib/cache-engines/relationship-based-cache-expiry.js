...
export function expireKeys(request, payload){
  ...
  if(action === 'create'){
    ...
    const belongsToModelNames = Object.values(model.belongsTo).map((model) => model.modelName);
    const hasManyModelNames = Object.values(model.hasMany).map((model) => model.modelName);

    const relationshipModelNames = new Set(belongsToModelNames.concat(hasManyModelNames));
